 ...
 if (isset($_POST['submit']))
        {
			...
			$que1=mysqli_query($conn,"SELECT * FROM `users` WHERE email = '$email' AND  password = '$password1' ");
			...
			else if($user == $tutor)
			{
					 ...
					 if ($conn->query($sql) === TRUE)
					 {
					  $sqlpassword = "INSERT INTO users (name, email, password) VALUES ('" . $firstname . "', '" . $email . "', '" . $password1 . "') ";
					  if ($conn->query($sqlpassword) === TRUE) 
					   {
						$_SESSION['email'] = $email;
						$email =  $_SESSION['email'];
					...
					//	$message .= "<img src='about-image.jpg' height="120" width="130" alt='upload'/></body></html>";
						...
						$message .= '<center><a href="DiceTutor/Login.php"  text-align:center;><button name="submit" class="form-control" style="color:#FF0000; background-color:#0066FF; text-align:center; height:55px; width:150px;">Procced To Login</button></a></center>';       
						$message .= '</form>';
						$message .= '<br>';
						//$message .= 'Verification Code :' . $verifyNo ;
						$message .= '<br>';
			...
			else if($user == $parent)
			{
				$sql = "INSERT INTO teacherregister (FirstName, LastName, Email, Password, Password1, VerifyNo) VALUES ('" . $firstname . "','" . $lastname . "', '" . $email . "', '" . $password1 . "', '" . $password2 . "', '" . $verifyNo . "') ";
					 ...
					 if ($conn->query($sql) === TRUE)
					 {
						  $sqlpassword = "INSERT INTO users (name1, email1, password) VALUES ('" . $firstname . "', '" . $email . "', '" . $password1 . "') ";
						  if ($conn->query($sqlpassword) === TRUE) 
					   {
						  $sqlpassword1 = "INSERT INTO  validcard (email) VALUES ('" . $email . "') ";
						  if ($conn->query($sqlpassword1) === TRUE) 
