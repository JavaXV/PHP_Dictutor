      ...
      <div class="form-inline">
        <div class="input-group" data-widget="sidebar-search">
           <form action="" method="POST">
            <div>
              <input type="text" name="search" class="form-control" placeholder="Tutor Id" style="width:150px;">
			  <input type="text" name="time" class="form-control" placeholder="Time E.g : 10AM TO 12PM" style="width:230px;" >
                <input type="submit" class="form-control" name="searchRequest" value="Search" style="color:#FFFFFF; background-color:#0033FF;">
              </div>
            </div>
          </form>
              <li class="nav-item">
                <a href="checkRequest.php" class="nav-link active" style="color:#FFFFFF;">
                  BACK
                </a>
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
			...
			<br>
		      <b style="color:#0033FF;">Note: Please take note of Tutor ID... required to check Request</b>
          </div><!-- /.col -->
...
$sql = "SELECT * FROM serviceadd WHERE radNo = '".$search."' AND timing = '".$time."'";
if($result = mysqli_query($link, $sql)){
    if(mysqli_num_rows($result) > 0){
      
         echo "<table style='position:absolute; color:black; left:70px; width:1450px;' class='table table-striped table-hover' border='1' ";
	         echo "<tr>";
                echo "<th>Tutor ID</th>";
				echo "<th>Location</th>";
                echo "<th>Confirmed-Request</th>";
                echo "<th>Price/Hrs</th>";
				echo "<th>State</th>";
				echo "<th>ServiceOption</th>";
                echo "<th>Gender</th>";
        ...
        while($row = mysqli_fetch_array($result)){
              $id = $row['id'] ;
			  echo '<p class="brand-text" style="font-size:25px; color:#FF0000;">'.'NOTE:  Total Payment for Duration of :' . '   '. $row['duration'] . '  To  ' . $row['duration2'] . ' is   : '   . $row['salary']."NGN ". '<a href="" style="color:#0000FF; font-size:16px;">' . 'Proceed To Payment'.'</a>'.'</p>' ;
       
			echo "<tr>";
             
	echo "<td>" . $row['radNo'] . "</td>";
	echo "<td>" . $row['city'] . "</td>";
	echo "<td>" . $row['requestaccept'] . "</td>";
	echo "<td>" . $row['price'] . "</td>";
	echo "<td>" . $row['state'] . "</td>";
	echo "<td>" . $row['serviceoption'] . "</td>";
	echo "<td>" . $row['gender'] . "</td>";
	echo "<td>" . $row['salary'] . "</td>";
	echo "<td>" . $row['subject'] . "</td>";
	echo  "<form action='showresult.php' method='POST'>";
	echo  "<input type='hidden' name='q' value='$id'>";
	echo "<td><img height='60' width='79' src='picture/".$row['image']."'><br><button name='search' align='center' style='background-color:blue; color:white;'>About Me</button></td>";
	echo  "</form>";
